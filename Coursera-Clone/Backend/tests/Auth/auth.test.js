...
const UserCycle=async ()=>{
    ...
    new_User = await User.create({name:'amaan',email:"amaansayyad2001@gmail.com",password:'password',
    isverified:true,courses:[], bookmark:[],preferences:[] });

    new_course = await Course.create({ title: "course_test", category:"Machine Learning",
    name:"amaan",discription:"something",creator:new_User._id});

    access_token = await jwt.sign({email:"amaansayyad2001@gmail.com",userId:new_User._id},global.access_token,{
    algorithm: "HS256",
    expiresIn:"10h"
    });                   
...
describe("Authentication" , ()=>{
    ...
    it("Signup", async()=>{
        ...
        .set(header)
        .send({email:"test@gmail.com", name:'Amaan', password:"password"})
        .expect(201)
        .then(res=>{
        ...
        .send({email:"amaansayyad2001@gmail.com",password:"password"})
        .expect(401)
        // .then(res=>{
